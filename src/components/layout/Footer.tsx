...
export function Footer() {
  return (
    <footer className="border-t border-ivory-linen bg-ivory-parchment px-8 py-16">
      <div className="mx-auto max-w-5xl">
        <div className="mb-10 grid grid-cols-2 gap-8 md:grid-cols-4 md:gap-12">
          {footerSections.map((section) => (
            <div key={section.title}>
              <h3 className="mb-4 text-xs font-semibold uppercase tracking-widest text-espresso">
                {section.title}
              </h3>
                ...
                {section.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      ...
                      className="text-sm text-gold-muted transition-colors hover:text-espresso"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        <div className="flex flex-col items-center justify-between gap-4 border-t border-ivory-linen pt-8 sm:flex-row">
