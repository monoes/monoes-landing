...
register(
  ...
  export function resolve(specifier, context, next) {
    ...
    if (specifier === "@/lib/db/schema") {
      return { url: "data:text/javascript,export const feature = {};", shortCircuit: true };
    }
...
describe("status validation", () => {
  it("isValidStatus accepts open, planned, shipped, declined only", async () => {
    const { isValidStatus } = await import("./route.ts");
    assert.equal(isValidStatus("open"), true);
    assert.equal(isValidStatus("planned"), true);
    assert.equal(isValidStatus("shipped"), true);
    assert.equal(isValidStatus("declined"), true);
