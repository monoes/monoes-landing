...
import { orgComment } from "@/lib/db/schema";
...
export async function DELETE(
  ...
  const [existingComment] = await db
    .select({ id: orgComment.id, authorId: orgComment.authorId })
    .from(orgComment)
    .where(and(eq(orgComment.id, commentId), eq(orgComment.orgUploadId, id)))
    .limit(1);
  ...
  await db.delete(orgComment).where(eq(orgComment.id, commentId));
